...
import { PlotLayerValueSnapshot, RocChartPlotData, RocChartPlotLayer, RocChartPlotMarker, RocPlotsVisibility } from '@models/roc';
...
export class LifetimeEarningsChartComponent implements OnChanges
{
  ...
  update(
    chartPlotData: RocChartPlotData
  ): void
  {
    // Create the X axis:
    this.xScale.domain(
    ...
    if (chartPlotData.currentState)
    {
      this.updateCurrentState(chartPlotData.currentState);
    }

    if (chartPlotData.goalState)
    {
      this.updateGoalState(chartPlotData.goalState);
    }
  ...
  private updateGoalState(goalStateLayer: RocChartPlotLayer): void
  {
    // Create a update selection: bind to the new data
    const u = this.svg
      .selectAll('.goalState-earningsMedian')
      .data(
        [goalStateLayer.values],
  ...
  private updateCurrentState(currentStateLayer: RocChartPlotLayer): void
  {
    // Create a update selection: bind to the new data
    const u = this.svg
      .selectAll('.currentState-earningsMedian')
      .data(
        [currentStateLayer.values],
